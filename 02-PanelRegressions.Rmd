...
The $\varepsilon_{i,t}$'s are not observed but $\bv{b}$ is a consistent estimator of $\boldsymbol\beta$. Using an adjustment for the degrees of freedom, we can approximate their variance with:
$$
...
In the context of Randomized controlled trials (RCTs), entities are randomly assigned to receive the treatment. As a result, we have $\mathbb{E}(Y_1) = \mathbb{E}(Y_1|W=0) = \mathbb{E}(Y_1|W=1)$ and $\mathbb{E}(Y_0) = \mathbb{E}(Y_0|W=0) = \mathbb{E}(Y_0|W=1)$. Using thsi into Eqs. \@ref(eq:EY1a) and \@ref(eq:EY0a) yields $ATE = \alpha$.
...
The disturbance $\varepsilon_{i,t}$ affects the outcome, but we assume that it does not relate to the selection for treatment; therefore, $\mathbb{E}(\varepsilon_{i,t}|W_{i,t})=0$. By contrast, we do not exclude some correlation between $W_{i,t}$ (for $t=1$) and $\mu_i$; hence, $\mu_i$ may constitute a *confounder*.
...
```{r figMeyer, fig.align = 'left-aligned', out.width = "90%", fig.cap = "Source: Meyer et al., (1995).", echo=FALSE}
