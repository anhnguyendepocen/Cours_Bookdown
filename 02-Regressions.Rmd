...
The researcher is usually interested in the components of vector $\boldsymbol\beta$, denoted by $\beta_k$, $k \in \{1,\dots,K\}$. She usually aims at estimating these coefficients based on observations of $\{y_i,\bv{x}_{i}\}$, $i \in \{1,\dots,n\}$, which constitutes a **sample** (of size $n$).
...
```{r heteroskedasticity, echo=FALSE, fig.cap="Homoskedasticity vs heteroskedasticity.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
N <- 200
X <- rnorm(N)
eps <- rnorm(N)
...
::: {.proposition #Sigma}
If Hpoytheses \@ref(hyp:homoskedasticity) and \@ref(hyp:noncorrelResid) hold, then:
$$
...
In this section, we will present and study the properties of the very popular estimation approach called **Ordinary Least Squares (OLS)**. As suggested by its name, the OLS estimator of $\boldsymbol\beta$ is defined as the vector $\bv{b}$ that minimizes the sum of squared residuals. (The **residuals** are the estimates of the **errors** $\varepsilon_i$.)
...
We  have:
$$
\bv{y} = \underbrace{\bv{X}\bv{b}}_{\mbox{fitted values } (\bv{y})} + \underbrace{\bv{e}}_{\mbox{residuals}}
$$
...
It is easily checked that $\bv{X}'\bv{e}=0$. Each column of $\bv{X}$ is therefore orthogonal to $\bv{e}$. In particular, if an intercept is included in the regression ($x_{i,1} \equiv 1$, i.e., the first column of $\bv{X}$ is filled with ones), the average of the residuals is null.
...
$$
Therefore, we have $\mathbb{V}ar(w'\bv{b^*}|\bv{X})=w'\mathbb{V}ar(\bv{b}|\bv{X})w + \sigma^2 w'\bv{D}\bv{D}'w\ge w'\mathbb{V}ar(\bv{b}|\bv{X})w=\mathbb{V}ar(w'\bv{b}|\bv{X})$.
:::
...
```{r R2issue, echo=TRUE, fig.cap="This figure illustrates the monotonous increase in the $R^2$ as a function of the number of explanatory variables. In the true model, there is no explanatory variables, i.e., $y_i = \\varepsilon_i$. We then take (independent) regressors and regress $y$ on the latter, progresively increasing the set of regressors.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
...
=\mbox{Tr}(\bv{M}\mathbb{E}(\boldsymbol{\varepsilon}\boldsymbol{\varepsilon}'|\bv{X}))=\sigma^2 \mbox{Tr}(\bv{M})$. (Note that we have $\mathbb{E}(\boldsymbol{\varepsilon}\boldsymbol{\varepsilon}'|\bv{X})=\sigma^2Id$ by Assumptions \@ref(hyp:homoskedasticity) and \@ref(hyp:noncorrelResid), see Prop. \@ref(prp:Sigma).) Finally:
\begin{eqnarray*}
...
\end{equation}
where $t(n-K)$ denotes a $t$ distribution with $n-K$ degrees of freedom.[^FootnoteMarginal]
...
What precedes is widely exploited for statistical inference in the context of linear regressions. Indeed, Eq. \@ref(eq:resultstudentt) gives a sense of the distances between $b_k$ and $\beta_k$ that can be deemed as "likely". For instance, it implies that, if $\sqrt{v_k s^2}$ is equal to 1 (say), then the probability to obtain $b_k$ smaller than $\beta_k-$ `r round(qt(.9995,df=10),3)` $\times \sqrt{v_k s^2}$ or  larger than $\beta_k+$ `r round(qt(.9995,df=10),3)` $\times \sqrt{v_k s^2}$ is equal to 0.1\% when $n-K=10$.
...
$$
We recall that the **p-value** is defined as the probability that $|Z| > |t|$, where $t$ is the (computed) t statistics and where $Z \sim t(n-K)$. That is, the p-value is given by $2(1 - \Phi_{t(n-K)}(|t_k|))$. See [this webpage](https://jrenne.shinyapps.io/tests/) for details regarding the link between critical regions, p-value, and test outcomes.
...
For this purpose, we make use of Eq. \@ref(eq:resultstudentt), i.e., $t_k = \frac{b_k - \beta_k}{\sqrt{s^2v_k}} \sim t(n-K)$. We have:
\begin{eqnarray*}
...
By Proposition \@ref(prp:constrainedLS) (in Appendix \@ref(constrainedLS)), we have: $\bv{b}_*-\bv{b}=-(\bv{X}'\bv{X})^{-1} \bv{R}'\{\bv{R}(\bv{X}'\bv{X})^{-1}\bv{R}'\}^{-1}(\bv{R}\bv{b} - \bv{q})$. Therefore:
$$
...
Under regularity assumptions, and under Hypotheses \@ref(hyp:fullrank) to \@ref(hyp:noncorrelResid), even if the residuals are not normally-distributed, the least square estimators can be *asymptotically normal* and inference can be performed as in small samples when Hypotheses \@ref(hyp:fullrank) to \@ref(hyp:normality) hold. This derives from Prop. \@ref(prp:asymptOLS) (below). The F-test (Prop. \@ref(prp:Ftest)) and the t-test (Eq. \@ref(eq:resultstudentt)) can then be performed.
...
In practice, $\sigma^2$ is estimated with $\frac{\bv{e}'\bv{e}}{n-K}$ (Eq. \@ref(eq:s2)) and $\bv{Q}^{-1}$ with $\left(\frac{\bv{X}'\bv{X}}{n}\right)^{-1}$. That is, the covariance matrix of the estimator is approximated by:
\begin{equation}
\widehat{\mathbb{V}ar}(\bv{b}) = s^2 (\bv{X}'\bv{X})^{-1}.(\#eq:sXX)
\end{equation}
...
$$
(each column of $(\bv{X}_1'\bv{X}_1)^{-1}(\bv{X}_1'\bv{X}_2)$ are the OLS regressors obtained when regressing the columns of $\bv{X}_2$ on $\bv{X}_1$). Unless the variables included in $\bv{X}_1$ are orthogonal to those in $\bv{X}_2$, we obtain a bias.
...
The statistical inference presented above relies on strong assumptions regarding the stochastic properties of the errors. Namely, they are assumed to be mutually uncorrelated (Hypothesis \@ref(hyp:noncorrel_resid)) and homoskedastic (Hypothesis \@ref(hyp:homoskedasticity). 
...
Autocorrelation is, in particular, a recurrent problem when time-series data are used (see Section \@ref(TS)).
...
Consider the case presented in Example \@ref(exm:autocorrelaaa). Because the OLS estimate $\bv{b}$ of $\boldsymbol\beta$ is consistent, the estimates $e_i$s of the $\varepsilon_i$s also are. Consistent estimators of $\rho$ and $\sigma_v$ are then obtained by regressing  the $e_i$s on  the $e_{i-1}$s. Using these estimates in Eq. \@ref(eq:SigmaAutocorrel) provides a consistent estimate of $\boldsymbol\Sigma$. Applying these steps recursively gives an efficient estimator of $\boldsymbol\beta$ (@Cochrane_Orcutt_1949).
:::
...
The conditional covariance matrix of the OLS estimator is therefore not $\sigma^2 (\bv{X}'\bv{X})^{-1}$ any longer Therefore, using $s^2 (\bv{X}'\bv{X})^{-1}$ for inference may be misleading. Below, we will see appropriate how to construct appropriate estimates of the covariance matrix of $\bv{b}$. Before, that, let us prove that the OLS estimator remains consistent in the GRM framework.
...
:::
:::{.proof}
We have $\mathbb{V}ar(\bv{b})=\mathbb{E}[\mathbb{V}ar(\bv{b}|\bv{X})]+\mathbb{V}ar[\mathbb{E}(\bv{b}|\bv{X})]$. Since $\mathbb{E}(\bv{b}|\bv{X})=\boldsymbol\beta$, $\mathbb{V}ar[\mathbb{E}(\bv{b}|\bv{X})]=0$. Eq. \@ref(eq:xsx) implies that $\mathbb{V}ar(\bv{b}|\bv{X}) \rightarrow 0$. Hence $\bv{b}$ converges in mean square, and therefore in probability (see Prop. \@ref(implicationsconv)).
:::
...
:::{.proposition #AsymptIVGRM name="Asymptotic distribution of the OLS estimator in the GRM framework"}
...
The so-called **robust covariance matrices** are estimates of the previous matrix. Their computation is based on the fact that if $\bv{b}$ is consistent, then the $e_i$'s are consistent (pointwise) estimators of the $\varepsilon_i$'s. Let us present such robust covariance matrices in two basic situations: heteroskedasticity (Example \@ref(exm:HCheteroskedasticity)) and auto-correlation of the residuals (Example \@ref(exm:))

:::{.example #HCheteroskedasticity name="Heteroskedasticity"}

This is the case of Eq. \@ref(eq:heteroskedasticity).

We then need to estimate $\frac{1}{n}\sum_{i=1}^{n}\sigma_{i}^2\bv{x}_i\bv{x}'_i$. @White_1980:  Under general conditions:
\begin{equation}
...
\begin{equation}
\frac{1}{n}\bv{X}'\bv{E}^2\bv{X},(\#eq:White)
\end{equation}
...
\begin{equation}
\bv{b} - \boldsymbol\beta = \left(\sum_{g=1}^G \bv{X}_g'\bv{X}_g\right)^{-1}\bv{X}'\sum_{g=1}^G \bv{s}_g,(\#eq:cluster1)
\end{equation}
where $\bv{s}_g = \bv{X}_g'\boldsymbol\varepsilon_g$ denotes the score vector (of dimension $K \times 1$) associoated with the $g^{th}$ cluster.


If the model is correctly specified then $\mathbb{E}(\bv{s}_g))0$ for all clusters $g$. Note that Eq. \@ref(eq:cluster1) is valid for any partition of $\{1,\dots,n\}$. Nevertheless, dividing the sample into **clusters** really becomes meaningful if we assume that the following hypothesis holds:
...
Matrix $\Sigma_g$ depends on the covariance structure of the $\varepsilon$'s. In particular, if $\Omega_g = \mathbb{E}(\boldsymbol\varepsilon_g\boldsymbol\varepsilon_g'|\bv{X}_g)$, then we have $\Sigma_g = \mathbb{E}(\bv{X}_g'\Omega_g\bv{X}_g)$.

Under Hypothesis \@ref(hyp:cluster), it comes that the covariance matrix of $\bv{b}$ is:
\begin{equation}
\left(\bv{X}'\bv{X}\right)^{-1}\left(\sum_{g=1}^G \Sigma_g\right)\left(\bv{X}'\bv{X}\right)^{-1}(\#eq:cluster2)
\end{equation}

Let us denote by $\varepsilon_{g,i}$ the error associated with the $i^{th}$ component of vector $\boldsymbol\varepsilon_g$. Consider the special case where $\mathbb{E}(\varepsilon_{g,i} \varepsilon_{g,j}|\bv{X}_g)=\sigma^2\mathbb{I}_{\{i=j\}}$, then Eq. \@ref(eq:cluster2) gives the standard expression $\sigma^2\left(\bv{X}'\bv{X}\right)^{-1}$.

If we have $\mathbb{E}(\varepsilon_{gi} \varepsilon_{gj}|\bv{X}_g)=\sigma_{gi}^2\mathbb{I}_{\{i=j\}}$, then we fall in the case addressed by the White formula (see Eq. \@ref(eq:White)), i.e.:
$$
...
$$
The natural way to estimate Eq. \@ref(eq:cluster2) consists in replacing the $\Sigma_g$ by their sample equivalent, i.e. $\widehat{\Sigma}_g=\bv{X}_g'\bv{e}_g\bv{e}_g'\bv{X}_g$. Adding corrections for the degrees of freedom, this leads to the following estimate of the covariance matrix of $\bv{b}$:
\begin{equation}
\frac{G(n-1)}{(G-1)(n-K)}\left(\bv{X}'\bv{X}\right)^{-1}\left(\sum_{g=1}^G\widehat{\Sigma}_g\right) \left(\bv{X}'\bv{X}\right)^{-1}. (\#eq:AsymptCL)
\end{equation}
The previous estimate is CRCV1 in @MACKINNON2022.

Note that we indeed find the White estimator when $G=n$ (see Eq. \@ref(eq:White)).
...
$$
because $\bv{X}'\bv{e}=0$. Hence, large clusters not necessarily increase variance.
...
The asymptotic theory can be based on tow different approaches: (i) large number of clusters (common case), and (ii) fixed number of clusters but large number of observations in each cluster (see SUbsections 4.1 and 4.2 in @MACKINNON2022). The more variable the $N_g$'s, the less reliable asymptotic inference based on Eq. \@ref(eq:AsymptCL), especially when a very few clusters are unusually large, or when the distribution of the data is heavy-tailed (has fewer moments). These issues are somehow mitigated when the clusters have an approximate factor structure.
...
As an alternative to the asymptotic approximation to the distribution of a statistic of interest, one can resort to bootstrap approximation (see Section 5 of @MACKINNON2022). In R, the packge `fwildclusterboot` allows to implement such approaches (see, e.g., [this tutorial by Alexander Fischer](https://cran.r-project.org/web/packages/fwildclusterboot/vignettes/fwildclusterboot.html)).

## Shrinkage methods

Chosing the right variables is often complicated, especially in the presence of many potentially relevant covariates. Keeping a large number of covariates results in large standard deviations for the estimated parameters (see Section \@ref(irrelevant)). In order to address this issue, shrinkage methods have been designed. The objective of these methods is to help to select of a limited number of variables (by shrinking the regression coefficients of the less useful variables towards zero). The two best-known shrinkage techniques are **ridge regression** and the **lasso** approach.[^FootnoteLASSO]
...
\begin{equation}
\bv{b} = \underset{\boldsymbol\beta}{\mbox{argmin}}\; \sum_{i=1}^n(y_i - \bv{x}_i'\boldsymbol\beta)
\end{equation}
is replaced by the following:
\begin{equation}
\bv{b}_\lambda = \underset{\boldsymbol\beta}{\mbox{argmin}}\; \sum_{i=1}^n(y_i - \bv{x}_i'\boldsymbol\beta) + \lambda f(\boldsymbol\beta),(\#eq:minLasso)
\end{equation}
where $\lambda f(\boldsymbol\beta)$ is a penalty term that positively depends on the "size" of the comppments of $\boldsymbol\beta$. This term is called the *shrinkage penalty* term.

Specifically, assuming that vector $\bv{x}_i$, that contains the whole set of potential covariates, is of dimension $K \times 1$, we have:
\begin{eqnarray*}
...
Parameter $\lambda$ is a , that has to be determined separately from the minimization problem of Eq. \@ref(eq:minLasso). One can combine standard criteria (e.g., BIC or Akaike) with lasso regressions to help determine $\lambda$.

In R, one can use the `glmnet` package to run ridge and lasso regressions. In the folowing example, we employ this package to model the interest rates proposed to debtors. The data come from the [Lending Club](https://www.kaggle.com/datasets/ethon0426/lending-club-20072020q1).
...
plot(cvglmnet)
cvglmnet$lambda.min # value of lambda.min, that is the value of lambda that gives minimum mean cross-validated error
cvglmnet$lambda.1se # largest value of lambda that is such that the sample cost is within the +1/-1 standard-deviation band stemming from the cv procedure
coef(cvglmnet, s = "lambda.min") # associated parameters
predict(cvglmnet, newx = as.matrix(x)[1:5,], s = "lambda.min") # predicted values of y for specific values of x
```
