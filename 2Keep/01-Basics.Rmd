...
A continuous random variable $X$ admits the probability density function $f$ if:
$$
\mathbb{P}(a < X \le b) = \int_{a}^{b}f(x)dx, \quad \forall a \le b,
$$
where $f(x) \ge 0$ for all $x$.
:::


In particular (if this limit exists), we have:
\begin{equation}
...
$$
This [web interface](https://jrenne.shinyapps.io/density/) illustrates the link between pdf. and cdf. Nonparametric estimates of pdf are obtained by kernel-based methods (see this [extra material](https://www.dropbox.com/s/lxkllp9yewh98d8/Additional_Kernel.pdf?dl=0)).
...
If $X$ and $Y$ are continuous r.v., then the distribution of $X$ conditional on $Y=y$, is denoted by $f_{X|Y}(x,y)$ and satisfies:
$$
...
Consider two r.v., $X$ and $Y$, with c.d.f. and p.d.f. $F_X$, $F_Y$, $f_X$ and $f_Y$.
...
* If $X$ and $Y$ are uncorrelated, they are not necessarily independent (Example: $X=Y^2-1$ and $Y \sim\mathcal{N}(0,1)$).
...
$X$ is drawn from a mixture of Gaussian distributions if:
$$
X = \color{blue}{B \times Y_1} + \color{red}{(1-B) \times Y_2},
$$
where $B$, $Y_1$ and $Y_2$ are three independent variables:

$B \sim \mbox{Bernoulli}(p)$, $Y_1 \sim \mathcal{N}(\mu_1,\sigma_1^2)$ and $Y_2 \sim \mathcal{N}(\mu_2,\sigma_2^2)$. 

Figure \@ref(fig:mixtureG) displays the pdfs associated with three different mixtures of Gaussian distributions. This [web-interface](https://jrenne.shinyapps.io/density/) allows to produce the pdf associated for any parameterization.
...
Suppose we have a floor made of parallel strips of wood, each the same width [$w=1$], and we drop a needle onto the floor. What is the probability that the needle lands on a line?

Assumption: needles' length = 1/2.
...
Conditionally on $\theta$: $\mathbb{E}(X|\theta)=\cos(\theta)/2$.

$\theta$ is uniformly distributed on $[-\pi/2,\pi/2]$, therefore:
...
Consider the case of a mixture of Gaussian distributions (Example \@ref(exmpl:mixture)). We have:
\begin{eqnarray*}
...
Assume that $X_i \sim i.i.d. \mbox{Cauchy}$ with a location parameter of 1 and a scale parameter of 1 (Def. \@ref(def:Cauchy)). The sample mean $\bar{X}_n = \frac{1}{n}\sum_{i=1}^{n} X_i$ does not converge in probability. This is because a Cauchy distribution has no mean; hence the law of large numbers cannot apply.
