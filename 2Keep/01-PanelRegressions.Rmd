...
$$
where $\alpha = \mathbb{E}(\bv{z}'_{i}\boldsymbol\alpha)$ and $u_i = \bv{z}'_{i}\boldsymbol\alpha - \mathbb{E}(\bv{z}'_{i}\boldsymbol\alpha) \perp \bv{x}_i$. In that case, the OLS is consistent, but not efficient.
...
The LSDV approach (Eq. \@ref(eq:LSDV)) can still be resorted to. It suffices to extend the $\bv{Z}$ matrix with additional columns (*time dummies*):
\begin{equation}
...
library(AEC)
JST <- subset(JST,year>1950);N <- dim(JST)[1]
...
eq1 <- lm(dhpreal ~ stir + ltrate + iso + YEAR,data=JST)
eq2 <- lm(dhpreal ~ I(ltrate-stir) + iso + YEAR,data=JST)
library(sandwich);library(lmtest)
vcov_cluster1 <- vcovHC(eq1, cluster = JST[, c("iso","YEAR")])
vcov_cluster2 <- vcovHC(eq2, cluster = JST[, c("iso","YEAR")])
robust_se_FE1    <- sqrt(diag(vcov_cluster1))
robust_se_FE2    <- sqrt(diag(vcov_cluster2))
stargazer::stargazer(eq1, eq1, eq2, eq2, type = "text",
                     column.labels = c("no Cluster", "Clusters","no Cluster", "Clusters"),
                     omit = c("iso","YEAR"),no.space = TRUE,
                     omit.labels = c("Country FE","Year FE"),keep.stat = "n",
                     se = list(NULL,robust_se_FE1,NULL,robust_se_FE2))
```
...
Here, the individual effects are assumed to be not correlated to other variables (the $\bv{x}_i$'s).
...
The $\varepsilon_{i,t}$'s are not observed but $\bv{b}$ is a consistent estimator of $\boldsymbol\beta$. Using an adjustment for the degrees of freedom, we have:
$$
...
Let us come back to Example \@ref(exm:JSTPanel). In the following, we use the random effect specification; and compare the results with those from the pooled regression and the fixed-effect model. For that, we use the function `plm` of the package of the same name. Note that `eq.FE` is similar to `eq1` in Example \@ref(exm:JSTPanel).

```{r JSTpanel2, warning=FALSE}
library(plm)
eq.RE <- plm(dhpreal ~ stir + ltrate,data=JST,index=c("iso","YEAR"),model="random",
             effect="twoways")
eq.FE <- plm(dhpreal ~ stir + ltrate,data=JST,index=c("iso","YEAR"),model="within",
             effect="twoways")
eq0 <- plm(dhpreal ~ stir + ltrate,data=JST,index=c("iso","YEAR"),model="pooling") 
stargazer::stargazer(eq0, eq.RE, eq.FE, type = "text",no.space = TRUE,
                     column.labels = c("Pooled","Random Effect", "Fixed Effects"),
                     add.lines=list(c('State FE','No','Yes','Yes'),
                                    c('Year FE','No','Yes','Yes')),omit.stat=c("f","ser"))
```
...
This example makes use of Airbnb prices (Zürich, 22 June 2017), collected from [Tom Slee's website](http://tomslee.net/airbnb-data-collection-get-the-data). The covariates are the number of bedroonms and the number of people that can be accommodated. We consider the use of district fixed effects. Figure \@ref(fig:airbnb) shows the price to explain. (The size of the circles is proportional to prices.)

```{r airbnb, warning=FALSE, message=FALSE, fig.align = 'left-aligned', out.width = "95%", fig.cap = "Airbnb prices for the Zurich area, 22 June 2017. The size of the circles is proportional to prices.", echo=FALSE}
library(rgdal)
library(AEC)
library(sandwich)
...
[^footnote1]: The bias may vanish for large $T$'s, though.
...
```{r dynpanel1, fig.align = 'left-aligned', out.width = "95%", fig.cap = "illustration of the bias pertianing to the LSDV estimation approach in the presence of auto-correlation of the depend variable.", echo=FALSE}
n <- 500;T <- 10
rho <- 0.9;sigma <- .5
alpha <- rnorm(n)
...
To address this, one can resort to instrumental-variable regressions. @Anderson_Hsiao_1982 have, in particular, proposed a first-differenced Two Stage Least Squares (2SLS) estimator. This estimation is based on the following transformation of the model:
\begin{equation}
\Delta y_{i,t} = \rho \Delta y_{i,t-1} + (\Delta \bv{x}_{i,t})'\boldsymbol\beta + \Delta\varepsilon_{i,t}.(\#eq:paneldynFisrtDiff)
\end{equation}
The OLS estimates of the parameters are biased because $\varepsilon_{i,t_1}$ ---which is part of the error $\Delta\varepsilon_{i,t}$--- is correlated to $y_{i,t-1}$ ---which is part of the "explanatory variable", namely $\Delta y_{i,t-1}$. But consistent estimates can be obtained using 2SLS with instrumental variables that are both correlated with $\Delta y_{i,t}$ and orthogonal to $\Delta\varepsilon_{i,t}$. One can for instance use $\{y_{i,t-2},\bv{x}_{i,t-2}\}$ as instruments. Note that this approach can be implemented only if there are more than 3 time observations per entity $i$.
...
While the OLS estimate of $\rho$ (whose true value is `r rho`) was `r round(b[1],3)`, we obtain here `rho_TSLS` $=$ `r round(rho_TSLS,3)`.
...
$$
\mathbb{E}(Z_i'\Delta  \bv{v}_i)=0,
$$

with $\Delta\bv{v}_i = [ \Delta v_{i,3},\dots,\Delta v_{i,T}]'$.
...
$$
\mbox{argmin}\;\left(\frac{1}{n} \sum_{i=1}^n Z_i' \Delta \bv{v}_i\right)'W_n\left(\frac{1}{n} \sum_{i=1}^n Z_i' \Delta \bv{v}_i\right),
$$
using the weighting matrix
$$
W_n = \left(\frac{1}{n}\sum_{i=1}^n Z_i'\widehat{\Delta\bv{v}_i}\widehat{\Delta\bv{v}_i}'Z_i\right)^{-1},
$$
where the $\widehat{\Delta\bv{v}_i}$'s are consistent estimates of the $\Delta\bv{v}_i$'s that result from a preliminary estimation. In this sense, this estimator is a two-step GMM one.
...
@Arellano_Bond_1991 have proposed a specification test. If the model is correctly specified, then the errors of Eq. \@ref(eq:paneldynFisrtDiff) --that is the first-difference equation-- should feature non-zero first-order auto-correlation, but zero higher-order autocorrelation. 
...
This section brielfy introduces the econometrics of program evaluation. A comprehensive review is proposed by @Abadie_Cattaneo_2018. A seminal book on the subject is that of @angrist_mostly_2008.
...
$$
where $\mathbb{E}(\varepsilon_i|Z_i) = 0$ (and $W_i$ is independent from $Z_i$ and $\varepsilon_i$). In this case, we obtain a consistent estimate of $\beta_0$ by regressing $\bv{y}$ on $\bv{X} = [\bv{w},\bv{Z}]$.
...
The disturbance $\varepsilon_{i,t}$ affects the outcome, but it does not relate the selection for treatment; therefore, $\mathbb{E}(\varepsilon_{i,t}|W_{i,t})=0$. By contrast, we do not exclude some correlation between $W_{i,t}$ (for $t=1$) and $\mu_i$; hence, $\mu_i$ may consitute a **confounder**.
...
```{r figAbadie, fig.align = 'left-aligned', out.width = "95%", fig.cap = "Source: Abadie et al., (1998).", echo=FALSE}
...
As shown in Figure \@ref(figMeyer), the measure has only affected high-earning workers. The idea exploited by @Meyer_Viscusi_Durbin_1995 was to compare the increase in time out of work before-after 1980 for higher-earnings workers on the one hand (entities who received the treatment) and low-earnings workers on the other hand (control group).

```{r figMeyer, fig.align = 'left-aligned', out.width = "70%", fig.cap = "Source: Meyer et al., (1995).", echo=FALSE}
...
eq2 <- lm(log(durat) ~ afchnge + highearn + afchnge*highearn + lprewage*highearn + male + married + lage + ltotmed + hosp + indust + injtype,data=injury)
eq3 <- lm(log(durat) ~ afchnge + highearn + afchnge*highearn + lprewage*highearn + male + married + lage + indust + injtype,data=injury)
