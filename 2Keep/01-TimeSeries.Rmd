...
### PACF approach to identify AR/MA processes
...
\end{array}\right],
$$
where the $\gamma_j$'s are given by Eq. \@ref(eq:autocovMA). However, for large samples, the computation of the likelihood can be numerically demanding. Hence, the p.d.f. of $Y$ is given by (see Def. \@ref(def:multivarG)):
$$
...
If the dynamics of process $y_t$ can be described as a VARMA model, and if $y_t$ is covariance statinary (see Def. \@ref(def:covstat)), then $y_t$ admits the following infinite MA representation (MA($\infty$)):
\begin{equation}
...
$$
(Note thta this infinite sum exists as soon as Eq. \@ref(eq:condiInfiniteMA) is satisfied.)
...
Estimation data are quarterly, and span the period from 1950:2 to 1987:4; 8 lags are used in the VAR model.
...
### Identificagtion based on non-normality of the shocks {#NonGaussian}
