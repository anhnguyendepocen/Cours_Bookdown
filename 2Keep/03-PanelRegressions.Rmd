...
Let us now use the [Cigarette Consumption Panel dataset](https://rdrr.io/cran/AER/man/CigarettesSW.html) of [Stock and Watson (2007)](https://www.pearson.com/en-gb/subject-catalog/p/Stock-Introduction-to-Econometrics-Global-Edition-4th-Edition/P200000005500/9781292264523)
...
Date from @JST_2017 [see website](https://www.macrohistory.net). This is version 6.
...
```{r airbnb, warning=FALSE, message=FALSE}
library(rgdal)
library(AEC)
library(sandwich)
library(lmtest)
...
In several cases, auto-correlation in the variable of interest may stem from an auto-regressive specification of the variable of interest. Eq. \@ref(eq:panel1) is then replaced by:
\begin{equation}
...
The previous equation shows that the *within-group* estimator (LSDV) introduces all realisations of the $\varepsilon_{i,t}$ errors into the transformed error term ($\tilde{\varepsilon}_{i,t}$). As a result, in large $n$, fixed $T$ panels, it is consistent only if all the right-hand side variables are strictly exogenous (i.e., do not correlate to past, present, and future errors $\varepsilon_{i,t}$).[^footnote1]

[^footnote1]: The bias may vanish for large $T$'s.
...
@Anderson_Hsiao_1982 proposed a first-differenced Two Stage Least Squares (2SLS) estimator. This estimation is based on the following transformation of the model:
\begin{equation}
...
If the explanatory variables $\bv{x}_{i,t}$ are assumed to be predetermined (i.e., do not contemporaneous correlate with the errors $\varepsilon_{i,t}$), then $\bv{x}_{i,t-1}$ can be added to the instruments associated with $\Delta y_{i,t}$. Further, if these variables (the $\bv{x}_{i,t}$'s) are assumed to be exogenous (i.e., do not contemporaneous correlate with any of the errors $\varepsilon_{i,s}$, $\forall s$), then $\bv{x}_{i,t}$ also is a valid instrument.
...
For $t=3$, $y_{i,1}$ (and $\bv{x}_{i,1}$) is the only possible instrument. However, for $t=4$, one could use $y_{i,2}$ and $y_{i,1}$ (as well as $\bv{x}_{i,2}$ and $\bv{x}_{i,1}$). More generally, defining matrix $Z_i$ as follows:
$$
Z_i = \left[
...
$$
where the $\widehat{\Delta\bv{v}_i}$'s are consistent esitmates of the $\Delta\bv{v}_i$ that result from a preliminary estimation. Hence, this estimator is a two-step GMM one.
...
Hansen J-test of the overidentifying restrictions (see Section \@ref(overidentif)):
...
```{r figAbadie, fig.align = 'center', out.width = "90%", fig.cap = "Source: Abadie et al., (1998).", echo=FALSE}
...
```{r figMeyer, fig.align = 'center', out.width = "70%", fig.cap = "Source: Meyer et al., (1995).", echo=FALSE}
