...
The so-called **robust covariance matrices** are estimates of the previous matrix. Their computation is based on the fact that if $\bv{b}$ is consistent, then the $e_i$'s are consistent (pointwise) estimators of the $\varepsilon_i$'s. 
...
\bv{X}.(\#eq:White)
\end{equation}
where the $e_i$ are the OLS residuals of the regression. The previous estimator is often called **HC0**. The **HC1** estimator, due to @MacKinnon_White_1985, is obtained by applying an adjustment factor $n/(n-K)$ for the number of degrees of freedom (as in Prp. \@ref(prp:expects2)). That is:
\begin{equation}
...
In the following example, we regress GDP growth rates from the @JST_2017 database on a systemic financial crisis dummy. We compute the HC0- and HC1-based standard deviations of the parameter estimate, and compare it to the one based on the standard OLS formula.
...
all.b <- NULL;all.OLS.stdv.b <- NULL;all.Whi.stdv.b <- NULL;all.NW.stdv.b <- NULL
for(i in 1:nb.sim){
  eps <- rnorm(n);x <- rnorm(n)
  for(i in 2:n){
...
cbind(sd(all.b),mean(all.OLS.stdv.b),mean(all.Whi.stdv.b),mean(all.NW.stdv.b))
```
...
par(mfrow=c(1,2))
plot(data$year,data$stir,ylim=c(-10,20),type="l",lwd=2,xlab="",ylab="",main="Nominal rate and inflation")
...
The present section, based on @MACKINNON2022, explains how to compute estimates of $Q_{x\boldsymbol\Sigma x}=\mbox{plim }(\bv{X}'\boldsymbol\Sigma\bv{X}/n)$ (see Prop. \@ref(prp:AsymptGRM)) when the dataset can be decomposed into clusters. These clusters are such that residuals may be correlated within a given cluster, but not across different clusters. A cluster may, e.g., gathers entities from a given geographical area, from the same industry, or same age cohort. Another useful reference is @Cameron_Miller_2014.
...
If we have $\mathbb{E}(\varepsilon_{gi} \varepsilon_{gj}|\bv{X}_g)=\sigma_{gi}^2\mathbb{I}_{\{i=j\}}$, then we fall in the case addressed by the White formula (see Example \@ref(eq:HCheteroskedasticity)), i.e.:
$$
...
$$
As in @White_1980, the natural way to approach the conditional covariance given in Eq. \@ref(eq:cluster2) consists in replacing the $\Sigma_g$ matrices by their sample equivalent, i.e. $\widehat{\Sigma}_g=\bv{X}_g'\bv{e}_g\bv{e}_g'\bv{X}_g$. Adding corrections for the degrees of freedom, this leads to the following estimate of the covariance matrix of $\bv{b}$:
\begin{equation}
...
The asymptotic theory can be based on two different approaches: (i) large number of clusters (common case), and (ii) fixed number of clusters but large number of observations in each cluster (see Subsections 4.1 and 4.2 in @MACKINNON2022). The more variable the $N_g$'s, the less reliable asymptotic inference based on Eq. \@ref(eq:AsymptCL), especially when a very few clusters are unusually large, or when the distribution of the data is heavy-tailed. These issues are somehow mitigated when the clusters have an approximate factor structure.
...
Chosing the appropriate explanatory variables is often complicated, especially in the presence of many potentially relevant covariates. Keeping a large number of covariates results in large standard deviations for the estimated parameters (see Section \@ref(irrelevant)). In order to address this issue, shrinkage methods have been designed. The objective of these methods is to help to select a limited number of variables (by shrinking the regression coefficients of the less useful variables towards zero). The two best-known shrinkage techniques are **ridge regression** and the **lasso** approach.[^FootnoteLASSO]
...
The nature of the penalty (based on the $\ell_1$ or $\ell_2$ norms) implies a different behaviour of the parameter estimates when $\lambda$ --the *tuning parameter*-- grows. In the ridge regression, coefficient estimates go to zero (shrinkage); in the lasso case, some coefficients reach zero when $\lambda$ reach some values. In other words, while ridge regression achieve shrinkage, lasso regressions achieve shrinkage *and* variable selection.

Parameter $\lambda$ has to be determined separately from the minimization problem of Eq. \@ref(eq:minLasso). One can combine standard criteria (e.g., BIC or Akaike) with lasso regressions to help determine $\lambda$.
