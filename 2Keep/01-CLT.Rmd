...
The law of large numbers (LLN) and the central limit theorem (CLT) are two fundamental theorems of probability. The former states that the sample mean converges to the population mean (when it exists). The latter states that the distribution of the sum (or average) of a large number of independent, identically distributed (i.i.d.) variables with finite variance is approximately normal, regardless of the underlying distribution (as long as it features a finite variance). These theorems are at the basis of most statistical procedures.
...
Most of the time, it is necessary to use samples for research, because it is impractical to study the whole population. Typically, it is rare to observe the population mean, and we often use instead sample means (or average). For instance, the sample average of $\{x_1,\dots,x_n\}$ is given by:
$$
\overline{x}_n = \frac{1}{n}\sum_{i=1}^n x_i.
$$
This sample mean is an estimator of the true (population) mean $\mathbb{E}(x_i)$ (assuming the latter exists). According to the law of large numbers (Theorem \@ref(thm:LLN)), this estimator is *consistent*.

:::{.theorem #LLN name="Law of large numbers"}
The sample mean is a consistent estimator of the population mean. In other words, the sample mean converges to the population in probability (see Def. \@ref(def:convergenceproba)).
:::
:::{.proof}
...
Suppose one wants to know the average number of doctor consultations over 12 months, for Swiss people. One can get an estimate by computing the sample average of a large dataset, for instance using the [Swiss Household Panel (SHP)](https://forscenter.ch/projects/swiss-household-panel/):


```{r incomeDistri, fig.cap="Distribution of the number of doctor visits.", fig.align = 'left-aligned'}
library(AEC) # to load the shp data-frame.
Nb.doct.visits <- shp$p19c15
Nb.doct.visits <- Nb.doct.visits[Nb.doct.visits>=0]
par(plt=c(.15,.95,.2,.95))
barplot(table(Nb.doct.visits),xlim=c(0,25))
```

This average is equal to `r round(mean(Nb.doct.visits),2)`.
:::
...
A first answer is provided by the Chebychev inequality. Assume $x_i \sim i.i.d.\,\mathcal{L}(\mu,\sigma^2)$, where $\mathcal{L}(\mu,\sigma)$ denotes a distribution of mean $\mu$ and variance $\sigma^2$. We have:
	$$
	...
	$$
The Chebychev inequality (Theorem \@ref(thm:chebychev)) states that, for any $\varepsilon > 0$ (even very small), we have:
	$$
	...
	$$
Consider for instance the estimation of the average number of doctor visits. In this example, the sample length was $n=$ `r length(Nb.doct.visits)` and the variance $\sigma^2$ was approximately equal to `r round(var(Nb.doct.visits),2)`. Therefore, taking $\varepsilon=0.5$, we have that $\mathbb{P}(\bar{x}_n-0.5<\mu<\bar{x}_n+0.5)$ is lower than $\frac{\sigma^2}{n\varepsilon^2} \approx$
...
According to the CLT, when $n$ is large and whatever the distribution $\mathcal{L}$ of the $x_i$'s (as long as it features an average $\mu$ and a variance $\sigma^2$), the error between $\bar{x}_n$ and $\mu$ can be seen as a random variable that is normally distributed with a mean of zero and a standard deviation of $\sigma/\sqrt{n}$. We say that the convergence rate is *in $\sqrt{n}$*. The knowledge of the quantiles of the normal distribution can then be used to compute approximate confidence intervals for $\mu$ (based on the sample $\{x_1,/dots,x_n\}$).

This [web interface](https://jrenne.shinyapps.io/StatEcoII/) illustrtes the CLT with simulations (select the "CLT" block).


The CL theorem was first postulated by the mathematician Abraham de Moivre. In an article published in 1733, he used the normal distribution to approximate the distribution of the number of heads resulting from many tosses of a fair coin. This finding was nearly forgotten until Pierre-Simon Laplace recalled it in his[Th\'eorie analytique des probabilit\'es](https://archive.org/details/thorieanalytiqu01laplgoog), published in 1812. Laplace expanded De Moivre's finding by approximating the binomial distribution with the normal distribution.


:::{.exercise #ExoBuffon}
Consider Buffon's experiment (Example \@ref(exm:Buffon)). We have seen that $\pi$ can be estimated by counting the fraction of times the needles cross the the number of times the needles pass through the grooves of the floor.^[More precisely, the expectation of this fraction is equal to $1/\pi$.] Using the CLT, determine the minimal number $n$ of needles that one has to throw to get an estimate $\hat\pi$ ($= 1/\bar{X}_n$) of $\pi$ that is such that there is a 95\% chance that $\pi \in [\hat\pi-0.01,\hat\pi+0.01]$?
:::
...
If $\bv{x}_n$ is an i.i.d. sequence of $m$-dimensional random variables with mean $\boldsymbol\mu$ and variance $\Sigma$, where $\Sigma$ is positive definite and finite, then:
$$
...
* Income sample for men: $\{m_1,\dots,m_{n_m}\}$.
* Income sample for women: $\{w_1,\dots,w_{n_w}\}$.
...
Because $\bar{m}$ and $\bar{w}$ are based on randomly selected samples, they are independent (Def. \@ref(def:independent)) and:
	$$
