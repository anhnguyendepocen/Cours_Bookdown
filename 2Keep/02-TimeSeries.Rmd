...
The process $\{\varepsilon_t\}_{t \in] -\infty,+\infty[}$ is a white noise if, for all $t$ (a) $\mathbb{E}(\varepsilon_t)=0$, (b) $\mathbb{E}(\varepsilon_t^2)=\sigma^2<\infty$ and (c) for all $s\ne t$, $\mathbb{E}(\varepsilon_t \varepsilon_s)=0$.
:::

Another shocks that are commonly used are Martingale Difference Sequences:
...
A whote noise process is not necessarily a MDS. This is for instance the following process:
$$
...
We have: $L^2 y_t = y_{t-2}$ and, generally, $L^k y_t = y_{t-k}$.

Consider a time series $y_t$ defined by $y_t = \mu + \phi y_{t-1} + \varepsilon_t$, where the $\varepsilon_t$ are i.i.d.\,$\mathcal{N}(0,\sigma^2)$. Using the lag operator, the dynamics of $y_t$ can be expressed as follows:
$$
...
$$
where $f_{Y_t}$ is the unconditional (or marginal) density of $y_t$.
...
:::{.definition #covstat name="Covariance --or weak-- stationarity"}
...
```{r nonstat1, echo=FALSE, fig.cap="Example of a process that is not covariance stationary ($y_t = 0.1t + \\varepsilon_t$, where $\\varepsilon_t \\sim \\mathcal{N}(0,1)$).", fig.asp = .6, out.width = "90%", fig.align = 'center'}
T <- 200
y <- 0.1*(1:T) + rnorm(T)
...
```{r nonstat2, echo=FALSE, fig.cap="Example of a process that is covariance stationary but not strictly stationary. The red lines delineate the 99\\% confidence interval of the standard normal distri. ($\\pm 2.58$).", fig.asp = .6, out.width = "90%", fig.align = 'center'}
T <- 2000
y <- rnorm(T)
...
```{r autocov, echo=FALSE, fig.cap="Annual growth rate of Swiss GDP, based on the Jord\\`a-Schularick-Taylor Macrohistory Database.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
library(AEC)
data <- subset(JST,iso=="CHE")
par(plt=c(.1,.95,.1,.95))
T <- dim(data)[1]
data$growth <- c(NaN,log(data$gdp[2:T]/data$gdp[1:(T-1)]))
plot(data$year,data$growth,type="l",xlab="",ylab="")
...
```{r autocov2, echo=FALSE, fig.cap="For order $j$, the slope of the blue line is, approximately, $\\hat{\\gamma}_j/\\widehat{\\mathbb{V}ar}(y_t)$, where hats indicate sample moments.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
y <- data$growth
j <- 1;y_1 <- c(rep(NaN,j),y[1:(T-j)])
j <- 2;y_2 <- c(rep(NaN,j),y[1:(T-j)])
j <- 3;y_3 <- c(rep(NaN,j),y[1:(T-j)])
par(mfrow=c(1,2))
plot(y_1,y,xlim=c(-.3,.3),ylim=c(-.3,.3),main="(a) order: one");grid()
abline(lm(y~y_1),col="blue")
plot(y_3,y,xlim=c(-.3,.3),ylim=c(-.3,.3),main="(b) order: three");grid()
...
:::{.proof}
By Proposition \@ref(prp:absMs), Eq. \@ref(eq:TCL2) implies Eq. \@ref(eq:TCL20). For Eq. \@ref(eq:TCL2), see Appendix \@ref(proofTVTCL). For Eq. \@ref(eq:TCL4ts), see @Anderson_1971, p. 429.
:::
...
Loosely speaking, Theorem \@ref(thm:CLTcovstat) says that, for a given sample size, the higher the "persistency" of a proicess, the lower the accuracy of the sample mean as an estimate of the population mean. To illustrate, consider three processes that feature the same marginal variance (one, say), but different autocorrelations: 0, 80\%, and 99\%. Figure \@ref(fig:TVTCL) displays such three processes. It indeed appears that, the larger the autocorrelation of the process, the further the sample mean (dashed red line) from the population mean (red solid line).

Identical simulations can be performed using [this ShinyApp](https://jrenne.shinyapps.io/MacroEc/) (use panel "AR(1)").

```{r TVTCL, echo=FALSE, fig.cap="The three samples have been simulated using the following data generating process: $x_t = \\mu + \\rho (x_{t-1}-\\mu) + \\sqrt{1-\\rho^2}\\varepsilon_t$, where $\\varepsilon_t \\sim \\mathcal{N}(0,1)$. Case A: $\\rho = 0$;  Case B: $\\rho = 0.9$;  Case C: $\\rho = 0.99$. In the three cases, $\\mathbb{E}(x_t)=\\mu=2$ and $\\mathbb{V}ar(x_t)=1$.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
all.rho <- c(0,.7,.999)
par(plt=c(0.15,0.95,0.2,0.75))
par(mfrow=c(3,1))
count <- 0
for(rho in all.rho){
  count <- count + 1
...
If $\mathbb{E}(\varepsilon_t^2)=\sigma^2$, the unconditional mean and variances of $y_t$ are:
$$
...
Higher-order auto-covariances are zero ($\gamma_j=0$ for $j>1$). Therefore: A MA(1) process is covariance-stationary (Def. \@ref(def:covstat)).
...
```{r simMA, echo=TRUE, fig.cap="Simulation of MA processes.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
library(AEC)
T <- 100;nb.sim <- 1
y.0 <- c(0)
c <- 1;phi <- c(0);sigma <- 1
...
What if $k$ gets infinite? The notion of **infinite-order Moving Average process** exists and is important in time series analysis. The (infinite) sequence of $\theta_j$ has to satisfy some conditions for such a process to be well-defined (see Theorem \@ref(thm:infMA) below). These conditions relate to the "summability" of $\{\theta_{i}\}_{i\in\mathbb{N}}$.
...
:::{.proof}
See Appendix 3.A in Hamilton. By "well behaved", we mean $\Sigma_{i=0}^{T} \theta_{t-i} \varepsilon_{t-i}$ converges in mean square (Def. \@ref(def:convergenceLr)) to some random variable $Z_t$. The proof makes use of the fact that:
$$
...
i. $y_t = \mu + \sum_{i=0}^{+\infty} \theta_{i} \varepsilon_{t-i}$ exists (Theorem \@ref(them:infMA)) and is such that:
\begin{eqnarray*}
\mathbb{E}(y_t) &=& \mu\\
...
:::{.proof}
The absolute summability of $\{\theta_{t}\}$ and the fact that $\mathbb{E}(\varepsilon^2)<\infty$ implies that the order of integration and summation is interchangeable (see Hamilton, 1994, Footnote p. 52), which proves (i). For (ii), see end of Appendix 3.A in Hamilton (1994).
:::


### Auto-Regressive (AR) processes

:::{.definition #AR1 name="First-order AR process"}
...
The AR(1) process, as defined in Def. \@ref(defn:AR1), is cov.-stationary iff $|\phi|<1$.
:::
***
...
As we will see, the covariance-stationarity of $F$ hinges on matrix $F$ defined as:
\begin{equation}
F = \left[
\begin{array}{ccccc}
...
Indeed, the fact that process $y_t$ follows Eq. \@ref(eq:AR) is equivalent to the fact that 
$$
\bv{y}_t = F \bv{y}_{t-1} + \boldsymbol\xi_t
$$
with
$$
\boldsymbol\xi_t =
...
ii. The eigenvalues of $F$ (as defined Eq. \@ref(eq:F)) lie within the unit circle.
...
Process $y_t$ is cov.-stationary iff $\bv{y}_{t}$ also is cov.-stationary, which is the case iff all $z_{i,t}$, $i \in [1,p]$, are cov.-stationary. By Prop. \@ref(prp:statioAR1), process $z_{i,t}$ is cov.-stationary iff $|\lambda_i|<1$. This proves that (i) is equivalent to (ii).

Prop. \@ref(prp:Feigen) further proves that (ii) is equivalent to (iv). Finally, it is easily seen that (iii) is equivalent to (iv) (as long as $\phi_p \ne 0$).
:::


Using the lag operator (see Eq \@ref(eq:lagOp)), if $y_t$ is a cov.-stationary AR(p) process (Def. \@ref(def:ARp)), we can write:
$$
y_t = \mu + \psi(L)\varepsilon_t,
$$
where
\begin{equation}
\psi(L) = (1 - \phi_1 L - \dots - \phi_p L^p)^{-1} \quad and \quad \mu = \mathbb{E}(y_t) = \dfrac{c}{1-\phi_1 -\dots - \phi_p}.(\#eq:EAR)
\end{equation}
...
\right.(\#eq:gammas)
\end{equation}
Using $\gamma_j = \gamma_{-j}$ (Prop. \@ref(prp:gammaMinus)), one can solve for $(\gamma_0,\gamma_1,\dots,\gamma_{p})$ as functions of $(\sigma^2,\phi_1,\dots,\phi_p)$.
...
Eq. \@ref(eq:ARMA_wold) is the **Wold representation** of this ARMA process (see Theorem \@ref(thm:Wold) below).

The stationarity of the process depends only on the AR specification. If the process is stationary, the weights in $\psi(L)$ decay at a geometric rate.

We have seen that the $k^{th}$-order auto-correlation of a MA(q) process is null if $k>q$.

This is exploited, in practice, to determine the order of a MA process. There exists an equivalent tool for the AR processes; it is based on partial auto-correlations (see definition below).

:::{.definition #partialAC name="Partial correlation"}
...
In a time series context, the **partial auto-correlation** (see Def. \@ref(def:partialAC)) $\phi_{h,h}$ is defined as the partial correlation of $y_{t+h}$ and $y_t$ given $y_{t+h-1},\dots,y_{t+1}$.
...
```{r pacf, echo=TRUE, fig.cap="ACF/PACF analysis of two processes (MA process on the left, AR on the right).", fig.asp = .6, out.width = "100%", fig.align = 'center'}
library(AEC)
par(mfrow=c(3,2))
par(plt=c(.2,.9,.2,.95))
theta <- c(1,2,1);phi=0
...
For an ARMA process, the Wold representation is given by Eq. \@ref(eq:ARMA_wold). It can be computed by recursively replacing the lagged $y_t$s in Eq. \@ref(eq:ARMApq). In this case, the deterministic component ($\kappa$) is null.


### Impulse Response Functions (IRFs) in ARMA models

Consider the following ARMA(p,q) process, as defined in Def. \@ref(def:ARMApq). Based on its associated sequence of whote noise ($\{\varepsilon_t\}$), let us construct a novel sequence of shocks $\{\tilde\varepsilon_t^{(s)}\}$:
$$
...
\right.
$$
We denote by $\{\tilde{y}_t^{(s)}\}$ the process following Eq. \@ref(eq:ARMApq) where $\{\varepsilon_t\}$ is replaced by $\{\tilde\varepsilon_t^{(s)}\}$. The time series $\{\tilde{y}_t^{(s)}\}$ is the (counterfactual) series $\{y_t\}$ that would have prevailed if $\varepsilon_t$ had been higher than its realised value at date $s$ (only).

What is the relationship between $\{y_t\}$ and $\{\tilde{y}_t^{(s)}\}$?
...
The object $\frac{\partial y_t}{\partial \varepsilon_{s}}$ is called the dynamic multiplier of $y_t$ w.r.t. $\varepsilon_{t}$ (of order $t-s$).

Let us consider the **Wold decomposition** of $\{y_t\}$, i.e. (see Theorem \@ref(thm:Wold) and Eq. \@ref(eq:ARMA_wold)) for its application to ARMA processes):
$$
...
$$
Therefore, for $t \ge s$, we have $\boxed{\dfrac{\partial y_t}{\partial \varepsilon_{s}}=\psi_{t-s}.}$

The sequence $\left\{\dfrac{\partial y_{t+h}}{\partial \varepsilon_{t}}\right\}_{h \ge 0} \equiv \left\{\psi_h\right\}_{h \ge 0}$ defines the **impulse response function (IRF)** of $y_t$ to the shock $\varepsilon_t$.
...
2. For $h \ge 0$, (recursively) apply Eq. \@ref(eq:ARMApq):
$$
...
:::{.proof}
This is obtained by apllying $\partial/\partial \varepsilon_{t}$ on both sides of (Eq. \@ref(eq:ARMApq)):
$$
y_{t+h} = c + \phi_1 y_{t+h-1} + \dots + \phi_p y_{t+h-p} + \varepsilon_{t+h} + \theta_1 \varepsilon_{t+h-1} + \dots + \theta_q \varepsilon_{t+h-q}.
$$
:::

Note that Proposition \@ref(prp:computPsi) constitutes a simple way to compute the MA($\infty$) representation of an ARMA process.


```{r IRFarma, echo=TRUE, fig.cap="IRFs of three processes. Process 1 (MA(2)): $y_t = \\varepsilon_t + \\varepsilon_{t-1} + \\varepsilon_{t-2}$. Process 2 (ARMA(1,2)): $y_{t}=0.6y_{t-1} + \\varepsilon_t + 0.5\\varepsilon_{t-1}$. . Process 3 (ARMA(4,2)): $y_{t}=0.5y_{t-3} + 0.4y_{t-4} + \\varepsilon_t + \\varepsilon_{t-1} + \\varepsilon_{t-2}$.", fig.asp = .6, out.width = "100%", fig.align = 'center'}
T <- 21 # number of periods for IRF
theta <- c(1,1,1);phi <- c(0);c <- 0
y.sim <- sim.arma(c,phi,theta,sigma=1,T,y.0=rep(0,length(phi)),nb.sim=1,make.IRF = 1)
...
y.sim <- sim.arma(c,phi,theta,sigma=1,T,y.0=rep(0,length(phi)),nb.sim=1,make.IRF = 1)
plot(0:(T-1),y.sim[,1],type="l",lwd=2,
     main="(b) Process 2",xlab="Time after shock on epsilon",ylab="",col="red")
theta <- c(1,1,1);phi <- c(0,0,.5,.4)
y.sim <- sim.arma(c,phi,theta,sigma=1,T,y.0=rep(0,length(phi)),nb.sim=1,make.IRF = 1)
plot(0:(T-1),y.sim[,1],type="l",lwd=2,
     main="(c) Process 3",xlab="Time after shock on epsilon",ylab="",col="red")
```
...
```{r IRFgdp1, echo=TRUE, fig.cap="(P)ACF analysis of Swiss GDP growth.", fig.asp = .6, out.width = "100%", fig.align = 'center'}
library(AEC)
data <- subset(JST,iso=="CHE")
par(plt=c(.1,.95,.1,.95))
T <- dim(data)[1]
...
Figure \@ref(fig:IRFgdp2) shows the IRFs based on the two respective specifications (MA and AR).

```{r IRFgdp2, echo=TRUE, fig.cap="Dynamic response of Swiss annual growth to a shock on the innovation $\\varepsilon_t$ at date $t=0$. The solid line corresponds to an AR(1) specification; the dashed line corresponds to a MA(2) specification.", fig.asp = .6, out.width = "100%", fig.align = 'center'}
# Fit an AR process:
...
What is the effect of a one-unit increase in $x_t$ on $y_t$? To address this question, this has to be formalized appropriately. Let us introduce two related sequences of values for $\{x\}$. Denote the first by $\{a\}^t$ and the second by $\{\tilde{a}\}^t$. Further, assume that $a_s = \tilde{a}_s$ for all $s \ne t$, and that $\tilde{a}_t = a_t+1$.

With these notations, we define $\frac{\partial y_{t+h}}{\partial x_t}$ as follows:
\begin{equation}
\frac{\partial y_{t+h}}{\partial x_t} := \mathbb{E}(y_{t+h}|\{x\} = \{\tilde{a}^t\}) - \mathbb{E}(y_{t+h}|\{x\} = \{a\}^t).(\#eq:dynmultX)
\end{equation}
...
Moreover, it has to be stressed that the definition of the dynamic multipliers (Eq. \@ref(eq:dynmultX)) does not allow for a potential persistency, in the $\{x\}$ process of the shock occuring on date $t$. Going in this direction would necessitate to model the dynamics of $x_t$.

The next example is based on data used in @Stock_Watson_2003 (Chapter 16). The objective is to study the influence of the number of freezing days on the price of orange juice.
...
plot(date,price,type="l",xlab="",ylab="",main="(a) Price of orange Juice")
plot(date,c(NaN,dprice),type="l",xlab="",ylab="",main="(b) Monthly pct Change (Y[t])")
plot(date,FJ$fdd,type="l",xlab="",ylab="",main="(c) Number of freezing days (X[t])")
```
...
```{r freez4, echo=TRUE, fig.cap="The solid (respectively dashed) line corresponds to the ARMAX(0,0,12) (resp. ARMAX(3,0,1)) model. The first model is esitmated by OLS, the second by MLE.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
nb.periods <- 20
...
In the following example, we make use of monetary shocks identified through high-frequency data (see @Gertler_Karadi_2015). This dataset comes from [Valerie Ramey's website](https://econweb.ucsd.edu/~vramey/research.html) (see @Ramey_2016_NBER).
...
```{r Ramey1fig, echo=FALSE, fig.cap="The blue line corresponds to monetary-policy shocks identified by means of the Gertler and Karadi (2015)'s approach (high-frequency change in Euro-dollar futures). The black slid line is the year-on-year growth rate of industrial production.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
...
Figure \@(fig:Ramey3) show the result IRF, with a 95\% confidence band. The code used to produce the confidence bands (i.e., to compute the standard deviation of the dynamic multipliers for the different horizons) is based on the Delta method. The codes arae available in Appendix \@ref(IRFDELTA).


```{r Ramey3, echo=FALSE, fig.cap="Response of industrial-production growth to monetary-policy shocks. Dashed lines correpsond to the $\\pm$  2-standard-deviation bands.", fig.asp = .6, out.width = "90%", fig.align = 'center'}
...
Consider the general case (of any time series); assume we observe a sample $\bv{y}=[y_1,\dots,y_T]'$. In order to implement ML techniques (see Section \@ref(MLE)), we need to evaluate the joint p.d.f. (then called likelihood) of $\bv{y}$, i.e., $\mathcal{L}(\boldsymbol\theta;\bv{y})$, where $\boldsymbol\theta$ is a vector of parameters that characterizes the dynamics of $y_t$. The Maximum likelihood estimate of $\boldsymbol\theta$ will then be given by:
$$
...
Process $y_t$ is Markovian if $f_{Y_t|Y_{t-1},Y_{t-2},\dots} = f_{Y_t|Y_{t-1}}$.
:::
...
Let us start with the Gaussian AR(1) process (which is Markovian):
$$
...
These expressins can be plugged in Eq. \@ref(eq:recursMLE). But what about $f_{Y_1}(y_1;\boldsymbol\theta)$? There exist two possibilities:

1. We use the marginal distribution: $y_1 \sim \mathcal{N}\left(\dfrac{c}{1-\phi_1},\dfrac{\sigma^2}{1-\phi_1^2}\right)$.
2. $y_1$ is considered to be deterministic. In a way, that means observation is "sacrificed".
...
\end{eqnarray*}
where $f_{Y_p,\dots,Y_{1}}(y_p,\dots,y_{1};\boldsymbol\theta)$is the marginal distribution of $\bv{y}_{1:p} := [y_p,\dots,y_1]$. The marginal distribution of $\bv{y}_{1:p}$ is Gaussian; it is therefore perfectly characterised by its mean and covariance matrix:
\begin{eqnarray*}
...
Consider the regression (where $y_t$ follows Eq. \@ref(eq:armaMLE)):
\begin{equation}
y_t = \boldsymbol\beta'\bv{x}_t + \varepsilon_t,(\#eq:OLSregARp)
\end{equation}
where $\bv{x}_t = [1,y_{t-1},\dots,y_{t-p}]'$ and $\boldsymbol\beta = [c,\phi_1,\dots,\phi_p]'$. 

The fact that $\bv{x}_t$ correlates to the $\varepsilon_s$ for $s<t$ implies that the OLS estimator $\bv{b}$ of $\boldsymbol\beta$ is biased in small sample. Indeed, we have:
\begin{equation}
...
However, asymptotically, the previous correlations vanish and we have:
...
Rearranging Eq. \@ref{eq:ols_ar1), we have:
$$
...
$$
If $j>0$, we have $\mathbb{E}(\varepsilon_t\varepsilon_{t-j}\bv{x}_t\bv{x}_{t-j}')=\mathbb{E}(\mathbb{E}[\varepsilon_t\varepsilon_{t-j}\bv{x}_t\bv{x}_{t-j}'|\varepsilon_{t-j},\bv{x}_t,\bv{x}_{t-j}])=$ $\mathbb{E}(\varepsilon_{t-j}\bv{x}_t\bv{x}_{t-j}'\mathbb{E}[\varepsilon_t|\varepsilon_{t-j},\bv{x}_t,\bv{x}_{t-j}])=0$. Note that we have $\mathbb{E}[\varepsilon_t|\varepsilon_{t-j},\bv{x}_t,\bv{x}_{t-j}]=0$ because $\{\varepsilon_t\}$ is an i.i.d. white noise sequence. If $j=0$, we have:
$$
\gamma^v_0 = \mathbb{E}(\varepsilon_t^2\bv{x}_t\bv{x}_{t}')= \mathbb{E}(\varepsilon_t^2) \mathbb{E}(\bv{x}_t\bv{x}_{t}')=\sigma^2\bv{Q}.
$$
The convergence in distri. of $\sqrt{T}(X'\boldsymbol\varepsilon/T)=\sqrt{T}\frac{1}{T}\sum_{t=1}^Tv_t$ results from Theorem \@ref(thm:CLTcovstat) (applied on $\bv{v}_t=\bv{x}_t\varepsilon_t$), using the $\gamma_j^v$ computed above.
:::
...
$$
Assume that one wants to recover the sequence of $\{\varepsilon_t\}$'s based on observed values of $y_t$ (from date 1 to date $t$). One can use the previous expression, but value should be used for $\varepsilon_0$? If one does not use the true value of $\varepsilon_0$ but 0 (say), one does not obtain $\varepsilon_t$ but only an estimate of it ($\hat\varepsilon_t$, say), with:
$$
...
Note that $\hat{\mathcal{L}}^*(\boldsymbol\theta;\bv{y})$ is a complicated nonlinear function of $\mu$ and $\theta$. Its maximization involves numerical optimization procedures.
...
Let us assume that this process is an **invertible MA process**. That is, we assume that the roots of:
\begin{equation}
...
$$
which implies that, if we knew all past values of $y_t$, we also know $\varepsilon_t$. In this case, we can consistently estimate the $\varepsilon_t$ by recursively computing $\hat\varepsilon_t$ as (for $t>0$):
$$
...
A consistent estimate of the conditional log-likelihood is still given by Eq. \@ref(eq:MALstar).

Note that we could determine the exact likelihood of an MA process. Indeed, vector $Y = [y_1,\dots,y_T]'$ is a Gaussian-distributed vector of mean $\boldsymbol\mu = [\mu,\dots,\mu]'$. Its p.d.f. is given by (see Def. \@ref(def:multivarG)):
$$
(2\pi)^{-T/2}|\boldsymbol\Omega|^{-1/2}\exp\left( -\frac{1}{2} (Y-\boldsymbol\mu)' \boldsymbol\Omega^{-1} (Y-\boldsymbol\mu)\right),
$$
where $\boldsymbol\Omega$ is of the form:
$$
...
\end{array}\right],
$$
where the $\gamma_j$s are given by Eq. \@ref(eq:autocovMA). However, for large samples, the computation of the likelihood can be numerically demanding.
...
The previouss section explains how to fit a given ARMA specification. But how to choose an appropriate specification? A possibility is to employ the (P)ACF approach (see Figure \@ref(fig:pacf)). However, the previous approach leads to either an AR or a MA process (bit not an ARMA process). If one wants to consider various ARMA(p,q) specifications, for $p \in \{1,\dots,P\}$ and $q \in \{1,7dots,Q\}$, say, then one can resort to **information criteria**.

In general, when choosing a specification, one faces the following dilemma:

a. Too rich a specification may lead to "overfitting"/misspecification, implying additional estimation errors (in forecasts).
...
$$
with  $(i) \in\{AIC,HQ,BIC\}$ and where $\hat{\boldsymbol\theta}_T(k)$ denotes the ML estimate of $\boldsymbol\theta_0(k)$ -- a vector of parameters of length $k$. For $q \ge 0$, the model specified by $k$ parameters is a particular case of a model of $k+q$ parameters.
...
The lag suggested by criterion $(i)$ is given by:
$$
...
$$
* Case $k > k_0$: The likelihood ratio (LR) test results imply that: $2 \left(\log \mathcal{L}(\hat{\boldsymbol\theta}_T(k);\bv{y})-\log \mathcal{L}(\hat{\boldsymbol\theta}_T(k_0);\bv{y})\right) \sim \chi^2(k-k_0)$.
...
To illustrate, consider a linear regression with normal disturbances:
$$
