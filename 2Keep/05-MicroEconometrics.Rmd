...
If one component of $\bv{x}_i$ ($\forall i$) is 1 (intercept), then one of the $\alpha_j$ ($j\in\{1,\dots,J-1\}$) is not identified. (One can then arbitrarily set $\alpha_1=0$, which is done in the binary logit/probit cases.)
...
library(AEC)
library(MASS)
credit$emp_length_low5y   <- credit$emp_length %in% c("< 1 year","1 year","2 years","3 years","4 years")
...
pred.grade <- predict(model1,newdata = training) # predicted grade (the most likely according to model)
...
### General multinomial model {#MNL}


Regressors may be **alternative-specific** or **alternative invariant** (see also Section  \@ref(Avregressors)).

Fishing data: prices and catch rates are alternative-specific, income is alternative-invariant.

Notations: as in Section  \@ref(Avregressors), that is ...

* $\bv{u}_{i,j}$: vector of variables associated with agent $i$ and alternative $j$ (alternative-specific regressors).

Example: Travel time per type of transportation (transportation choice), wage per type of work, cost per type of car.
* $\bv{v}_{i}$: vector of variables associated with agent $i$ but alternative-invariant.

Example: age or gender of agent $i$,

We may e.g. organize $\bv{x}_i$ as follows:
\begin{equation}
\bv{x}_i = [\bv{u}_{i,1}',\dots,\bv{u}_{i,J}',\bv{v}_{i}']'.(\#eq:xorganiz)
\end{equation}

**General** Multinomial Logit
...
Note: By construction, $g_j(\bv{x}_i;\boldsymbol\theta) \in [0,1]$ and $\sum_{j}g_j(\bv{x}_i;\boldsymbol\theta)=1$.

If $\bv{x}_i$ organized as in Eq. (\@ref(eq:xorganiz)), then, with obvious notations, $\theta_j$ is of the form:
\begin{equation}
...
Remark: The labelling "CL" and "MNL" --used in the literature-- are relatively *ad hoc* (see 15.4.1 in @Cameron_Trivedi_2005).

The following lines replicate Table 15.2 in @Cameron_Trivedi_2005.
...
Consider the general model described in Eq. \@ref(eq:generalMultiNom). It can be noted that:
$$
...
Let's use the notation $p_{i,j} \equiv \mathbb{P}(y_i=j|\bv{x}_i;\boldsymbol\theta)$. According to the general logit formulation (Eq. \@ref(eq:GeneralMNL)), we have:
\begin{eqnarray*}
...
More general models can be defined. Notably by resorting to **Additive Random Utility Models, ARUM** (see Section \@ref(latent)).
...
**Utility Models and the Multinomial Logit**

:::{.definition #Gumbel name="Gumbel distribution"}
Gumbel distribution ($\mathcal{W}$), support: $\mathbb{R}$ (see Theorem \@ref(thm:Extreme)):
$$
F(u) = \exp(-\exp(-u)), \qquad f(u)=\exp(-u-\exp(u)).
$$
:::

If $X\sim\mathcal{W}$, then $\mathbb{E}(X)=0.577$ (Euler constant)[^FootnoteEuler] and $\mathbb{V}ar(X)=\pi^2/6$.

[^FootnoteEuler]: The Euler constant $\gamma$ satisfies $\gamma = \lim_{n\rightarrow \infty} \left(- \ln(n) + \sum_{k=1}^n \frac{1}{k}\right)$.

```{r Gumbel, echo=FALSE, fig.cap="C.d.f. of the Gumbel distribution ($F(x)=\\exp(-\\exp(-x))$).", fig.asp = .6, out.width = "90%", fig.align = 'center'}
x <- seq(-6,6,by=.01)
y <- exp(-exp(-x))
...
Case where $V_{i,j} = \theta_j'\bv{x}_i = \boldsymbol\beta'\bv{u}_{i,j}+{\theta_j^{(v)}}'\bv{v}_i$ (see Eqs.\,\@ref(eq:xOrganiz) and \@ref(eq:thetaOrganizCL)): we can assume that
$$
\begin{array}{clll}
(A) & \bv{u}_{i,1}&=&0,\\
(B) & \theta_1^{(v)} &=& 0.
...
If $J=2$ and $j \in \{0,1\}$ (shift by one unit), we have $\mathbb{P}(y=1|\bv{x})=\dfrac{\exp(\boldsymbol\theta'\bv{x})}{1+\exp(\boldsymbol\theta'\bv{x})}$, this is the logit model (Table \@ref(tab:foo)).
...
That is, a Multinomial Logit can be seen as a series of pairwise comparisons that are unaffected by the characteristics of alternatives. Such a model is said to satisfy the **independence from irrelevant alternatives (IIA)** property.
...
i. The deterministic part of the utility is given by:
$$
...
\end{eqnarray}
where $I_j$'s are **inclusive values** or **log sums**, given by:
$$
...
b. $\rho_j=1 \Rightarrow$, $\varepsilon_{j,k}$ and $\varepsilon_{j,l}$ are uncorrelated ($\Rightarrow$ Multinomial logit).
c. When $J=1$:
$$
F([\varepsilon_1,\dots,\varepsilon_K]',\rho) = \exp\left(-\left(\sum_{k=1}^{K} \exp(-\varepsilon_k/\rho)\right)^{\rho}\right).
$$

The following figure displays simulations of pairs $(\varepsilon_1,\varepsilon_2)$ drawn from GEV distributions for different values of $\rho$. The simulation approach is based on [Bhat](http://www.caee.utexas.edu/prof/bhat/ABSTRACTS/Supp_material.pdf). The code used to produce this chart is in Appendix \@ref(App:GEV).

```{r GEV, fig.align = 'center', out.width = "90%", fig.cap = "GEV simulations.", echo=FALSE}
...
Travel model dataset used, e.g., by @Hensher_Greene_2002 (see also @Heiss_2002).
...
nl1 <- mlogit(choice ~ gcost + wait + incomeother, TravelMode,
              ...
              alt.var='mode', # This is the variable that defines the alternative choices.
              nests=list(public=c('train', 'bus'),
                         car='car',
                         air='air'), # The different components of the list define the "limbs".
              un.nest.el = TRUE
)
...
```{r tobit1, echo=FALSE, fig.cap="Bias in the case of sample selection. The grey line represents the population regression line. The model is $y_i = x_i + \\varepsilon_i$, with $\\varepsilon_{i,t} \\sim \\mathcal{N}(0,1)$. The red line is the OLS regression line based on black dots only.", fig.align = 'center'}
beta <- 1
sigma <- 1
N <- 100
eps <- rnorm(N) * sigma
...
The **Tobit, or censored/truncated normal regression model** correspinds to the case described above, but with Gaussian errors $\varepsilon$. Specifically:
$$
y^* = \boldsymbol\beta'\bv{x} + \varepsilon,
$$
...
Results usually heavily rely on distributional assumptions (more than in uncensored/untruncated case). The framework is easy to extend to an heteroskedastic case, for instance by setting $\sigma_i^2=\exp(\alpha'\bv{x}_i)$.


```{r tobit2, echo=FALSE, fig.cap="Censored dataset with heteroskedasticitiy. The model is $y_i = x_i + \\varepsilon_i$, with $\\varepsilon_{i,t} \\sim \\mathcal{N}(0,\\sigma_i^2)$ where $\\sigma_i = \\exp(-1 + x_i)$.", fig.align = 'center'}
alpha.0 <- -1
alpha.1 <- 1
x <- rnorm(N)
...
&=&\underbrace{\boldsymbol\beta'\bv{x}}_{=\mathbb{E}(y^*)} + \underbrace{\mathbb{E}(\varepsilon|\varepsilon>-\boldsymbol\beta'\bv{x})}_{>0} > \mathbb{E}(y^*).
\end{eqnarray*}
...
&=&  \boldsymbol\beta'\bv{x} + \sigma \underbrace{\frac{\phi(\boldsymbol\beta'\bv{x}/\sigma)}{\Phi(\boldsymbol\beta'\bv{x}/\sigma)}}_{=: \lambda(\boldsymbol\beta'\bv{x}/\sigma)} \nonumber\\
&=& \sigma \left( \frac{\boldsymbol\beta'\bv{x}}{\sigma} + \lambda\left(\frac{\boldsymbol\beta'\bv{x}}{\sigma}\right)\right). (\#eq:Econdtruncated)
\end{eqnarray}
...
```{r tobit3, echo=FALSE, fig.cap="Conditional means of $y$ in Tobit models. The model is $y_i = x_i + \\varepsilon_i$, with $\\varepsilon_i \\sim \\mathcal{N}(0,1)$.", fig.align = 'center'}
x <- seq(-4,4,by=.01)
...
2. Using the truncated sample only: run an OLS regression of $\bv{y}$ on $\left\{\bv{x},\lambda(\boldsymbol\beta'\bv{x}/\sigma)\right\}$. THis provides a consistent estimate of $(\boldsymbol\beta,\sigma)$.
...
where "Conditional mean" comes from Eq. \@ref(eq:Econdtruncated) and "disturbance" is an error of zero conditional mean.

This approach is also applied to the case of **sample selection models** (see Section\,\@ref(section:SSM}).

See 16.10.2 of @Cameron_Trivedi_2005 for the derivation of asymptotic standard errors of $\boldsymbol\beta$.

The following example are based on the dataset used in @Mroz_1987 (this dataset is part of the `sampleSelection` package).
...
```{r tobit5, echo=FALSE, fig.cap="Predicted versus observed wages.", fig.align = 'center'}
...
In the standard Tobit framework, the model determining censored --or truncated-- data **censoring mechanism** is the same as the one determining non-censored --or observed-- data **outcome mechanism**.

A **two-part model** adds flexibility by permitting the zeros and non-zeros to be generated by different densities.

Here, the second model characterizes the outcome *conditional on* the outcome being observed.


In a seminal paper, @Duan_et_al_1983 eploy this methodology to account for individual annual hospital expenses. The two models are then as follows:
...
The situation tackled by sample-selection models is the following. The dependent variable of interest, denoted by $y_2$, depends on observed variables $\bv{x}_2$. The fact that $y_2$ is observed depends on the value of a latent variable ($y_1^*$) that is correlated to observed variables $\bv{x}_1$. The difference w.r.t. the two-part model skethed above is that, even conditionally on $(\bv{x}_1,\bv{x}_2)$, $y_1^*$ and $y_2$ may be correlated.
...
&=& \frac{1}{\sigma}\phi\left(\frac{y_2 - \bv{x}_2'\boldsymbol\beta_2}{\sigma}\right) \times \nonumber\\
&& \mathbb{P}(y_1^*>0|y_2,\bv{x};\theta).(\#eq:probaPP2)
\end{eqnarray}

Let's compute $\mathbb{P}(y_1^*>0|y_2,\bv{x};\theta)$. By Prop. \@ref(prop:update) (in Appendix \@ref(GaussianVar)), applied to ($\varepsilon_1,\varepsilon_2$), we have:
$$
y_1^*|y_2 \sim \mathcal{N}\left(\bv{x}_1'\boldsymbol\beta_1 + \frac{\rho}{\sigma_2}(y_2-\bv{x}_2'\boldsymbol\beta_2),1-\rho^2\right).
$$
which leads to
\begin{equation}(\#eq:probaPP3)
\mathbb{P}(y_1^*>0|y_2,\bv{x};\theta) = \Phi\left( \frac{\bv{x}_1'\boldsymbol\beta_1 + \dfrac{\rho}{\sigma_2}(y_2-\bv{x}_2'\boldsymbol\beta_2)}{\sqrt{1-\rho^2}}\right).
\end{equation}

Figure \@ref(fig:SampleSelec) shows $\mathbb{P}(y_1^*>0|y_2,\bv{x};\theta)$ for different values of $y_2$ and of $\rho$, in the case where $\boldsymbol\beta_1=\boldsymbol\beta_2=0$.

```{r SampleSelec, echo=FALSE, fig.cap="Probability of observing $y_2$ depending on its value, for different values of conditional correlation between $y_2$ and $y_1^*$.", fig.align = 'center'}
...
Using Eqs. \@ref(eq:probaPP1), \@ref(eq:probaPP2) and \@ref(eq:probaPP3) gives the log-likelihood function:
\begin{eqnarray*}
...
&=& \bv{x}_2'\boldsymbol\beta_2 + \rho\sigma_2\frac{\phi(\bv{x}_1'\boldsymbol\beta_1)}{\Phi(\bv{x}_1'\boldsymbol\beta_1)}\nonumber\\
&=&\bv{x}_2'\boldsymbol\beta_2 + \rho\sigma_2\lambda(\bv{x}_1'\boldsymbol\beta_1).(\#eq:y2y11)
\end{eqnarray}
...
In particular, Eq. (\@ref(eq:y2y11) shows that $\mathbb{E}(y_2^*|y_1=1,\bv{x}) \ne  \bv{x}_2'\boldsymbol\beta_2$ when $\rho \ne 0$ $\Rightarrow$ biased OLS estimates based only on the sub-sample where $y_1=1$.

**Heckman two-step procedure** (or ``Heckit''): replace $\lambda(\bv{x}_1'\boldsymbol\beta_1)$ appearing in Eq. \@ref(eq:y2y11) by an estimate. More precisely:
...
$$
\begin{eqnarray*}
&&\mathbb{V}ar(y_2|y_1^*>0,\bv{x}) \\
&=& \sigma_2^2(1-\rho^2) + \rho^2\sigma_2^2 (1 - \bv{x}_1'\boldsymbol\beta_1 \lambda(\bv{x}_1'\boldsymbol\beta_1) - \lambda(\bv{x}_1'\boldsymbol\beta_1)^2)\\
...
The computation of parameter standard errors is complicated because of the two steps (see @Cameron_Trivedi_2005), Subsection 16.10.2). Bootstrap can be resorted to.

Let us use the @Mroz_1987 dataset again,with the objective of explaining wage setting.
...
heckvan <- heckit( lfp ~ age + I( age^2 ) + kids5 + huswage + educ, # selection equation
...
stargazer(logitW,probitW,heckvan,ml,type = "text",
          selection.equation = TRUE)
# Print outcome-equation estimates:
stargazer(ols1,heckvan,ml,type = "text")
```
...
The Poisson regression is intrinsically heteroskedastic (since $\mathbb{V}ar(y_i) = \exp[\boldsymbol\beta'\bv{x}_i]$).
...
$$
\widehat{\beta}_j \frac{1}{n}\sum_{i=1}^n  \exp(\boldsymbol\beta'\bv{x}_i),
$$
...
Hence, the p.d.f. of $y_i$ conditional on $\exp(\boldsymbol\beta'\bv{x}_i)=\mu$ and $\delta$ is obtained  as a mixture of densities:
$$
...
$$
the previous expression is the basis of the so-called **NB2** specification.

If $\delta$ is replaced by $\mu/\gamma$, then we get the **NB1** model:
$$
...
```{r countdata1, warning=FALSE, message=FALSE}
library(AEC)
#library(foreign)
library(COUNT)
library(pscl) # for predprob function and hurdle model
#randdata <- subset(randdata,year==1) # randdata included in AEC 
plot(table(randdata$mdvis))
```

```{r countdata2, warning=FALSE, message=FALSE}
randdata$LC <- log(1 + randdata$coins)
model.OLS <- lm(mdvis ~ LC + idp + lpi + fmde + physlm + disea + hlthg + hlthf + hlthp - 1,
                data=randdata)
model.poisson <- glm(mdvis ~ LC + idp + lpi + fmde + physlm + disea + hlthg + hlthf + hlthp - 1,
                     data=randdata,family = poisson)
model.neg.bin <- glm.nb(mdvis ~ LC + idp + lpi + fmde + physlm + disea + hlthg + hlthf + hlthp - 1,
                        data=randdata)
model.neg.bin.with.intercept <- glm.nb(mdvis ~ LC + idp + lpi + fmde + physlm + disea + hlthg + hlthf + hlthp,
                                       data=randdata)
stargazer::stargazer(model.OLS,model.poisson,model.neg.bin, model.neg.bin.with.intercept,type="text",no.space = TRUE)
```

Models' predictions can be obtained as follows:

```{r countdata3, warning=FALSE, message=FALSE}
predict_poisson.beta.x <- predict(model.poisson) # prediction of beta'x, equivalent to "model.poisson$fitted.values"
predict_poisson <- predict(model.poisson,type="response") # prediction of the number of events (exp(beta'x))
...
model.hurdle <- hurdle(mdvis ~ LC + idp + lpi + fmde + physlm +
                         disea + hlthg + hlthf + hlthp, data=randdata,
                       dist = "poisson", zero.dist = "binomial", link = "logit")
model.zeroinfl <- zeroinfl(mdvis ~ LC + idp + lpi + fmde + physlm +
                             disea + hlthg + hlthf + hlthp, data=randdata,
                           dist = "poisson", link = "logit")
stargazer(model.hurdle,model.zeroinfl,zero.component=FALSE,no.space=TRUE,type="text")
stargazer(model.hurdle,model.zeroinfl,zero.component=TRUE,no.space=TRUE,type="text")
```
