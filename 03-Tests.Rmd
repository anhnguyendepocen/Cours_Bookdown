...
* a **test statistic**, that is a function of the components of a sample ($S(\bv{x})$, where $\bv{x}=\{x_1,\dots,x_n\}$, say), and
...
* **Type I error**: Reject $H_0$ while it is true = False Positive (FP).
* **Type II error**: Absence of rejection of $H_0$ while it is false = False Negative (FN).


:::{.example #FPFN name="Early Warning Signals (practical illustration of type-I and type-II errors)"}

Early warning systems (EWS) are approaches designed to warn policy makers of potential future economic and financial crises. See, e.g., [ECB (2014)](http://www.ecb.europa.eu/events/pdf/conferences/140623/Vasicek-et-al_Comparing-Different-Early-Warning-Systems.pdf?F96bbb525a26071ecf97f9154fb3cc73)
...
Consider the case where, under $H_0$, the distribution of the test statistic is symmetrical (as in the case of the normal distribution or of the Student-t distribution). In this case, the critical region is usually defined by the union of the two tails of the distribution. The test is then said to be a **two-tailed test** or a **two-sided test**. This situation is illustrated by Figures \@ref(fig:Illusttest1) and \@ref(fig:Illusttest2). (Use this [web interface](https://jrenne.shinyapps.io/tests/) to explore alternative situations.)

Figure \@ref(fig:Illusttest2) also illustrates the notion of *p-value* (in the case of a two-sided test). The p-value can be defined as the value of the size of the test $\alpha$ that is such that the computed test statistic, $S$, is at the "frontier" of the critical region. Given this definition, if the p-value is smaller (respoectively larger) than the size of the test, we reject (resp. cannot rejet) the null hypothesis at the $\alpha$ significance level.
...
Figures \@ref(fig:IllusTestOneSided1) and \@ref(fig:IllusTestOneSided2) illustrate the **one-tailed**, or **one-sided** situation. These tests are tyxpically employed when the distribution of the test statistic under the null hypothesis has a support on $\mathbb{R}^+$ (e.g. $\chi^2$).
...
## A practical illustration of size and power
...
[web interface](https://jrenne.shinyapps.io/Factory/).





```{r Factory, echo=FALSE, fig.cap="Factory example.", fig.align = 'left-aligned'}
...
$k^{th}$ **central sample moment** of $Y$:
	$$
	m_k = \frac{1}{n}\sum_{i=1}^n(y_i - \bar{y})^k.
	$$
$k^{th}$ **standardized sample moment** of $Y$:
	$$
	g_k = \frac{m_k}{m_2^{k/2}}.
	$$

:::{.proposition #conssitCentralMoments name="Consistency of central sample moments"}
If the $k^{th}$ central moment of $Y$, exists, then the sample central moment $m_k$ is a consistent estimate of the central moment $\mu_k$.
:::


:::{.proposition #Asymptg3Normal name="Asymptotic distribution of 3rd-order sample central moment of a normal distribution"}
If $Y\sim\mathcal{N}(\mu,\sigma^2)$, then $\sqrt{n}g_3 \overset{d}{\rightarrow} \mathcal{N}(0,6)$.
:::
:::{.proof}
...
If $Y\sim\mathcal{N}(\mu,\sigma^2)$, then $\sqrt{n}(g_4-3) \overset{d}{\rightarrow} \mathcal{N}(0,24)$.
:::
...
Asymptotically, the vector $(\sqrt{n}g_3,\sqrt{n}(g_4-3))$ is bivariate Gaussian. Further its elements are uncorrelated and therefore independent.
:::
...
If $Y$ is Gaussian, $JB \overset{d}{\rightarrow} \chi^2(2)$.
:::
:::{.proof}
...
Now, replace `rnorm` with `runif`. That is, let us now consider what happens for the distribution of the JB statistics when the $y_i$'s are drawn from a uniform distribution. Figure \@ref(fig:JBTest3) then shows that, when $n$ grows, the distributions shift to the right. This results in the consistency of the JB test (see Def. \@ref(def:asmyptconsisttest)).
