...
```{r nonstat1, echo=FALSE, fig.cap="Example of a process that is not covariance stationary ($y_t = 0.1t + \\varepsilon_t$, where $\\varepsilon_t \\sim \\mathcal{N}(0,1)$).", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
T <- 200
y <- 0.1*(1:T) + rnorm(T)
...
```{r nonstat2, echo=FALSE, fig.cap="Example of a process that is covariance stationary but not strictly stationary. The red lines delineate the 99\\% confidence interval of the standard normal distribution ($\\pm 2.58$).", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
T <- 2000
y <- rnorm(T)
...
```{r autocov, echo=FALSE, fig.cap="Annual growth rate of Swiss GDP, based on the Jord\\`a-Schularick-Taylor Macrohistory Database.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
library(AEC)
...
```{r autocov2, echo=FALSE, fig.cap="For order $j$, the slope of the blue line is, approximately, $\\hat{\\gamma}_j/\\widehat{\\mathbb{V}ar}(y_t)$, where hats indicate sample moments.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
y <- data$growth
...
```{r simMA, echo=TRUE, fig.cap="Simulation of MA processes.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
library(AEC)
T <- 100;nb.sim <- 1
y.0 <- c(0)
c <- 1;phi <- c(0);sigma <- 1
...
In a time series context, the partial auto-correlation ($\phi_{h,h}$) of process $\{y_t\}$ is defined as the partial correlation of $y_{t+h}$ and $y_t$ given $y_{t+h-1},\dots,y_{t+1}$.
:::
...
\right.
$$
We denote by $\{\tilde{y}_t^{(s)}\}$ the process following Eq. \@ref(eq:ARMApq) where $\{\varepsilon_t\}$ is replaced by $\{\tilde\varepsilon_t^{(s)}\}$. The time series $\{\tilde{y}_t^{(s)}\}$ is the (counterfactual) series $\{y_t\}$ that would have prevailed if $\varepsilon_t$ had been shifted by $\delta$ on date $s$ (only change).
...
Let us consider the **Wold decomposition** of $\{y_t\}$ (see Theorem \@ref(thm:Wold) and Eq. \@ref(eq:ARMAwold) for the application of this theorem to ARMA processes):
$$
...
:::
:::{.proof}
This is obtained by apllying the operator $\frac{\partial}{\partial \varepsilon_{t}}$ on both sides of Eq. \@ref(eq:ARMApq):
$$
...
One can use function `sim.arma`, with the argument `make.IRF = 1` to compute ARMA's IRFs:
...
plot(date,c(NaN,dprice),type="l",xlab="",ylab="",main="(b) Monthly pct Change (y)")
plot(date,FJ$fdd,type="l",xlab="",ylab="",main="(c) Number of freezing days (x)")
```
...
```{r freez4, echo=TRUE, fig.cap="Response of changes in orange juice price (in percent) to the number of freezing days. The solid (respectively dashed) line corresponds to the ARMAX(0,0,12) (resp. ARMAX(3,0,1)) model. The first model is estimated by OLS, the second by MLE.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
nb.periods <- 20
...
plot(IRF1,type="l",lwd=2,col="red",xlab="months after shock",ylab="Chge in price (percent)")
...
```{r Ramey1fig, echo=FALSE, fig.cap="The blue line corresponds to monetary-policy shocks identified by means of the Gertler and Karadi (2015)'s approach (high-frequency change in Euro-dollar futures). The black slid line is the year-on-year growth rate of industrial production.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
...
```{r Ramey3, echo=FALSE, fig.cap="Response of industrial-production growth to monetary-policy shocks. Dashed lines correpsond to the $\\pm$  2-standard-deviation bands.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
...
The fact that $\bv{x}_t$ correlates to the $\varepsilon_s$'s for $s<t$ implies that the OLS estimator $\bv{b}$ of $\boldsymbol\beta$ is biased in small sample. Indeed, we have:
\begin{equation}
...
These two cases (exact or conditional log-likelihoods) can be implemented when asking R to fit an AR process by means of function `arima`. Let us for instance use the output gap of the `US3var` dataset (US quarterly data, from 1959:2 to 2015:1, used in @Gourieroux_Monfort_Renne_2017).
...
In this context, a consistent estimate of the conditional log-likelihood is still given by Eq. \@ref(eq:MALstar).
...
\end{array}\right],
$$
where the $\gamma_j$'s are given by Eq. \@ref(eq:autocovMA). However, for large samples, the computation of the likelihood can be numerically demanding. Hence, the p.d.f. of $Y$ is given by (see Prop. \@ref(prp:pdfMultivarGaussian)):
$$
...
The previouss section explains how to fit a given ARMA specification. But how to choose an appropriate specification? A possibility is to employ the (P)ACF approach (see Figure \@ref(fig:pacf)). However, the previous approach leads to either an AR or a MA process (bit not an ARMA process). If one wants to consider various ARMA(p,q) specifications, for $p \in \{1,\dots,P\}$ and $q \in \{1,\dots,Q\}$, say, then one can resort to **information criteria**.
...
```{r fcstARMA, warning=FALSE, fig.cap="XXX.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
library(AEC)
library(forecast)
data <- subset(JST,iso=="CHE")
T <- dim(data)[1]
...
```{r fcstVAR, warning=FALSE,message=FALSE,fig.cap="XXX.", fig.asp = .6, out.width = "90%", fig.align = 'left-aligned'}
library(vars)
