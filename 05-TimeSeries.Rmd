...
\end{array}\right],
$$
where the $\gamma_j$'s are given by Eq. \@ref(eq:autocovMA). The p.d.f. of $Y$ is then given by (see Prop. \@ref(prp:pdfMultivarGaussian)):
$$
...
Naturally, in practice, $\Omega$ is replaced with $\hat{\Omega}$, $Q$ is replaced with $\hat{\bv{Q}} = \frac{1}{T}\sum_{t=p}^T x_t x_t'$ and $\Sigma$ with the matrix whose components are of the form $\hat\sigma_{i,l}\hat\sigma_{j,m}+\hat\sigma_{i,m}\hat\sigma_{j,l}$, where the $\hat\sigma_{i,l}$'s are the components of $\hat\Omega$.
